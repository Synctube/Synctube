...
/**
 * Creates an observable map from a compatible event emitter.
 */

ObservableMap.fromEmitter = function (source) {
	return new Emitter(source);
};

function Emitter (source) {
	...
	source.on('move', function (key, before) {
		self.emit('move', key, before);
	});
	source.on('remove', function (key) {
		self.emit('remove', key);
	});
}

util.inherits(Emitter, ObservableMap);

/**
 * Creates an Emitter, without access to the underlying ObservableMap.
 */

ObservableMap.prototype.emitter = function () {
